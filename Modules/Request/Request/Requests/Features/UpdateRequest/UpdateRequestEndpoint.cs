...
public class UpdateRequestEndpoint : ICarterModule
{
    ...
    {
        app.MapPut("/requests", async (UpdateRequestRequest request, ISender sender) =>
        {
        ...
        .Produces<UpdateRequestResponse>(StatusCodes.Status200OK)
        .ProducesProblem(StatusCodes.Status400BadRequest)
        .ProducesProblem(StatusCodes.Status404NotFound)
        .WithSummary("Update an existing request")
        .WithDescription("Updates an existing request in the system. The request details are provided in the request body.");
    }
}
